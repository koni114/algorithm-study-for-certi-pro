...
class Solution:
    def findItinerary(self, tickets: List[List[str]]) -> List[str]:
        ...
        tickets_dict = {key: sorted(value_list) for key, value_list in tickets_dict}
            
            
        def recursive_dfs(curr_from, tickets_dict):
            results.append(curr_from)
            while tickets_dict[curr_from]:
                next_from = tickets_dict[curr_from]
                tickets_dict[curr_from].remove(next_from)
...
s = Solution()
s.findItinerary(tickets=tickets)